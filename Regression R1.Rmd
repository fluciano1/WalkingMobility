...
#Library imputeTS to impute values on moving averages (function: na_ma)
library(imputeTS)

#Libraryreadxl to read excel files
library(readxl)

#Library CoordinateCleaner to get capital city coordinates from country code
...
NOAAdf <- read.csv("NOAAdf_updated.csv", sep=",")
...
###########################################
#Export AMR as csv
write.csv2(AMR, file="dfAMR_T_prec.csv")
```
```{r}
############################################################
###               CENTER COVARIATES ON MEAN               ##
############################################################
#Create variables for centred temperature, daylight and precipitations
AMR$meanTavg <- NA
AMR$meanDaylight <- NA
AMR$meanPrec <- NA
...
for (c in unique(AMR$Code)){
  ...
  #Mean precipitation
  meanPrec <- mean(AMR[AMR$Code==c,]$Prec_mm)
  AMR$meanPrec[AMR$Code==c] <- meanPrec
...
AMR$Prec_mm_Centred <- AMR$Prec_mm-AMR$meanPrec
...
```{r}
#Model 3: mixed effect model for ln(walking) - random intercept with centred covariates, adjusting also for precipitations
lmm3c <- lmer(log(walking) ~ stringency_index + Daylight_Centred + AvrTemp_CelsiusDecimals_Centred + Prec_mm_Centred + (1|Code), data=AMR)
summary(lmm3c)
#coef(lmm2c)
confint(lmm3c)
#r2(lmm2c)
...
#Check model
check_singularity(lmm3c)
plot(check_collinearity(lmm3c))
plot(check_normality(lmm3c))
plot(check_heteroscedasticity(lmm3c))
```
